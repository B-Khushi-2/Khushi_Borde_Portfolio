...
async function testFiveQueriesWithLLM() {
  console.log(`\n======================================================================`);
  console.log(`TESTING 5 QUERY PATHS WITH SIMULATED LLM PROVIDER`);
  console.log(`======================================================================`);

  const queries = [
    {
      type: "1. Candidate-Fact (Obvious Keywords - Strong Match)",
      messages: [{ role: "user", content: "Tell me about Khushi Borde's Fire Detection CNN project" }]
    },
    {
      type: "2. Candidate-Fact (Obvious Keywords - General)",
      messages: [{ role: "user", content: "What experience does Khushi have with React and Python?" }]
    },
    {
      type: "3. Candidate-Fact (Vague / Paraphrased - Monolith)",
      messages: [{ role: "user", content: "did she work on breaking apart a monolith?" }]
    },
    {
      type: "4. Candidate-Fact (Vague / Paraphrased - Safety CV)",
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    // Pass mock provider to simulate active provider
    const result = await buildMessages(item.messages, "mock");
